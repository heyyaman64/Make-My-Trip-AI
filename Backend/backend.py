...
def itinerary_agent(state: TravelState):
    # Keep the itinerary prompt small enough for Groq's 8K TPM limit.
...
User Query:
{compact(state.get('user_query', ''), 1500)}

Trip Constraints:
{compact(state.get('trip_constraints', {}), 2000)}

Flight Results:
{compact(state.get('flight_results', ''), 3500)}

Hotel Results:
{compact(state.get('hotel_results', ''), 3500)}

Weather Results:
{compact(state.get('weather_results', ''), 2500)}

Budget Results:
{compact(state.get('budget_results', ''), 2500)}

Requirements:
- Create a practical day-by-day itinerary.
- Consider the available flight and hotel information.
- Consider weather conditions when planning activities.
- Respect the user's budget and trip constraints.
- Do not invent prices or availability.
- If information is unavailable, clearly say so.
- Keep the itinerary concise and easy to follow.
...
"""
        ...
        [
            SystemMessage(
                content=(
                    ...
                    "Create concise, realistic and budget-aware itineraries."
                )
            ),
            HumanMessage(content=prompt),
        ]
    ...
    return {
        ...
        "messages": [
            AIMessage(content="Draft itinerary created for human review.")
        ],
        "llm_calls": state.get("llm_calls", 0) + 1,
    }
...
def final_agent(state: TravelState):
    """
    Generate the final travel response using a compact version
    of the LangGraph state.

    Groq's on-demand TPM limit is 8000 tokens, so we intentionally
    limit the amount of previous agent output sent to the final LLM.
    """
    ...
    if state.get("approved", False):
        ...
        review_instruction = (
            "The user approved the draft itinerary. "
            "Preserve the important decisions and polish the final response."
        )
        ...
        review_instruction = (
            ...
            f"{feedback or 'Improve the draft before finalizing it.'}"
        )

    # ---------------------------------------------------------
    # Compact previous agent outputs
    # ---------------------------------------------------------
    ...
    trip_constraints = compact(
        state.get("trip_constraints", {}),
        1200
    )

    flight_results = compact(
        state.get("flight_results", ""),
        1800
    )

    hotel_results = compact(
        state.get("hotel_results", ""),
        1800
    )

    weather_results = compact(
        state.get("weather_results", ""),
        1200
    )

    budget_results = compact(
...
Create a clear and practical final response.
...
Rules:
- Preserve important decisions from the approved draft.
- Apply human feedback when revision was requested.
- Do not invent flight prices when pricing is unavailable.
- Mention when live API pricing is unavailable.
- Include useful weather and travel advice.
- Keep the response concise but useful.
- Do not repeat unnecessary raw API data.
"""
        ...
        [
            SystemMessage(
                content=(
                    ...
                    "Create concise, accurate and practical travel plans."
                )
            ),
